...
def index():
    ...
    # 根据某个字段获取一个dict出来 类别dict 从数据库获取指定条件的数据 id和FoodCat构成字典
...
def info():
    ...
    # 从‘数据库存变更表’查询该商品的库存变化
    stock_change_list = FoodStockChangeLog.query.filter( FoodStockChangeLog.food_id == id )\
        .order_by( FoodStockChangeLog.id.desc() ).all()

    resp_data['info'] = info  # 美食详细信息
    resp_data['stock_change_list'] = stock_change_list  # 库存变化信息
    resp_data['current'] = 'index'   # 美食列表
